...
After that, install `tensorflow` and `keras` in python using `keras` R package. Please make sure the `keras` R package's version is lower than 2.2.4.1. You can you `devtools` to install the correct version. You may also need to install Anaconda if you does not have it. For more information about installation of keras, please visit https://keras.rstudio.com/
```{r, eval=FALSE}
utils::install.packages('devtools')
devtools::install_version('keras', version = '2.2.4.1', repos = 'https://cran.rstudio.com/')
...
## Subtyping
We can use the main funtion `SCFA` to generate subtypes from multi-omics data. The input of this function is a list of matrices from different data types. Each matrix has rows as samples and cells as features.
```{r, eval=TRUE}
...
We can use the function `SCFA.class` to predict risk score of patients using available survival information from training data. We need to provide the function with training data with survival information, and testing data. 
```{r, eval=TRUE}
